...
var _ = g.Describe("[sig-etcd][apigroup:config.openshift.io][OCPFeatureGate:DualReplica][Suite:openshift/two-node][Serial] Fencing credentials", func() {
	...
	g.It("should not degrade when fencing is at risk but still available", func() {
		...
		unmanageCmd := fmt.Sprintf("sudo pcs resource unmanage %s", stonithResourceName)
		...
		g.DeferCleanup(func() {
			...
			manageCmd := fmt.Sprintf("sudo pcs resource manage %s 2>/dev/null; true", stonithResourceName)
		...
		manageCmd := fmt.Sprintf("sudo pcs resource manage %s", stonithResourceName)
